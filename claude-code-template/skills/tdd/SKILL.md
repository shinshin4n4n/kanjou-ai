---
name: tdd
description: Red-Green-Refactorサイクルでテスト駆動開発を実行する。「テスト駆動で」「TDDで実装して」「テストファーストで」等のリクエストで自動起動。
---
...
4. `{{TEST_UNIT_COMMAND}} -- {テストファイルパス}` を実行
...
3. `{{TEST_UNIT_COMMAND}} -- {テストファイルパス}` を実行
4. **全テストが通ることを確認**
5. テスト成功の出力をユーザーに表示
...
- テストが通らない状態でコミットしない
- カバレッジ {{COVERAGE_THRESHOLD}}% 以上を維持
